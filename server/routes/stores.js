...
router.get('/', async (req, res) => {
  try {
    const stores = await Store.find({ isActive: true })
      ...
      .select('name slug address phone district operatingHours sortOrder enableHikAccess enableRecharge isChainStore allianceEnabled');
    res.json({ stores });
  } catch (error) {
...
router.get('/by-slug/:slug', async (req, res) => {
  try {
    ...
    const store = await Store.findOne(filter)
      .select('name slug address phone district allianceEnabled branding operatingHours sortOrder enableHikAccess enableRecharge isChainStore');
    if (!store) {
      return res.status(404).json({ message: '店鋪不存在' });
    }
    res.json({ store });
