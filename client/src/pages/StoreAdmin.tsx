...
import { canAccessStoreAdmin, getMembershipRoleForStore } from '../utils/authRedirect';
...
type Tab = {
  ...
  /** staff 可見；其餘僅 manager／平台 admin */
  staffAllowed?: boolean;
};

function StoreAdminShell() {
  const { user, logout } = useAuth();
  const { storeSlug = '', store, loading, error, membershipRole } = useStoreAdmin();
  const [searchParams, setSearchParams] = useSearchParams();
  const [activeTab, setActiveTab] = useState('bookings');
  const [mobileOpen, setMobileOpen] = useState(false);

  const isStaffOnly = membershipRole === 'staff';

  const allTabs: Tab[] = useMemo(
    () => [
      { id: 'bookings', name: '預約管理', icon: CalendarDaysIcon, element: <BookingManagement />, staffAllowed: true },
      { id: 'calendar', name: '預約日曆', icon: CalendarDaysIcon, element: <BookingCalendar />, staffAllowed: true },
      { id: 'courts', name: '場地管理', icon: UserGroupIcon, element: <CourtManagement />, staffAllowed: true },
  ...
  const tabs = useMemo(
    () => (isStaffOnly ? allTabs.filter((t) => t.staffAllowed) : allTabs),
    [allTabs, isStaffOnly]
  );
  ...
  useEffect(() => {
    ...
    if (tabs.some((t) => t.id === tab)) {
      ...
      setActiveTab(tabs[0]?.id || 'bookings');
    }
  }, [searchParams, tabs]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <Link to="/" className="text-primary-600 mt-4 inline-block">返回首頁</Link>
        </div>
      </div>
    );
  }
